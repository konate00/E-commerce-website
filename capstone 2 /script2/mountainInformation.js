...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Event listener for mountain dropdown change
    mountainDropdown.addEventListener('change', function () {
        const selectedMountain = mountainDropdown.value.toLowerCase();
        const selectedMountainInfo = mountainsArray.find(mountain => mountain.name.toLowerCase() === selectedMountain);

        // Fetch sunrise/sunset times for the selected mountain
        getSunsetForMountain(selectedMountainInfo.coords.lat, selectedMountainInfo.coords.lng).then(data => {
            // Display mountain information including sunrise time
            displayMountainInfo(selectedMountainInfo, data.results);
        });
    });
});

// Function to fetch sunrise/sunset times for a specific mountain

...
window.onload = function () {
    ...
    const locationSearchDiv = document.getElementById('locationSearch');
    const parkTypeSearchDiv = document.getElementById('parkTypeSearch');

    // Set initial visibility
    // document.querySelector('#parkType').style.display = 'none';

    locationSearchDiv.style.display = 'none';
    parkTypeSearchDiv.style.display = 'none';


    searchButton.onclick = searchParksClick;
    viewAllButton.onclick = viewAllParksClick;

    searchByLocationRadio.addEventListener('change', toggleSearchOptions);
    searchByParkTypeRadio.addEventListener('change', toggleSearchOptions);

    const stateInput = document.querySelector('#state');
    const parkTypeInput = document.querySelector('#parkType');

    stateInput.onchange = searchParksClick;
    parkTypeInput.onchange = searchParksClick;
...
function toggleSearchOptions() {
    ...
    if (document.getElementById('searchByLocation').checked) {
        locationSearchDiv.style.display = 'block';
        parkTypeSearchDiv.style.display = 'none';
    } else if (document.getElementById('searchByParkType').checked) {
        locationSearchDiv.style.display = 'none';
        parkTypeSearchDiv.style.display = 'block';
    }
}
...
function loadLocations() {
    ...
    locationsArray.forEach(state => {
        ...
        locationOption.value = state.toLowerCase();
        locationOption.text = state;
        stateDropdown.appendChild(locationOption);
    });
}
...
parkTypesArray.forEach(type => {
    ...
    parkTypeOption.value = type.toLowerCase();
    parkTypeOption.text = type;
    parkTypeDropdown.appendChild(parkTypeOption);
});



function searchParksClick() {
    const stateInput = document.querySelector('#state').value.toLowerCase();
    const parkTypeInput = document.querySelector('#parkType').value.toLowerCase();

    // Check if either location or park type is selected
    if (stateInput !== "" || parkTypeInput !== "") {
        const filteredParks = nationalParksArray.filter(park => {
            const locationMatch = stateInput === "" || park.State.toLowerCase() === stateInput;
            const parkTypeMatch = parkTypeInput === "" || park.LocationName.toLowerCase().includes(parkTypeInput);
            return locationMatch && parkTypeMatch;
        });
        displayResults(filteredParks);
    } else {
        alert('Please select a location or park type.');
    }
}

function viewAllParksClick() {
    ...
    // If results are visible, display all national parks
...
function displayResults(results) {
    ...
    if (results.length === 0) {
        ...
        results.forEach(park => {
            ...
            parkContainer.classList.add('park-result');

            const parkInfo = `
            <p>${park.LocationName} - ${park.City}, ${park.State}</p>`;
            const visitLink = park.Visit ? `
            <a href="${park.Visit}" target="_blank">Visit Park Page</a>` : '';

            parkContainer.innerHTML = parkInfo + visitLink;
            resultsDiv.appendChild(parkContainer);
        });
    }
}
...
//         results.forEach(park => {
//             const parkInfo = `<p>${park.LocationName} - ${park.City}, ${park.State}</p>`;
//             const visitLink = park.Visit ? `<a href="${park.Visit}" target="_blank">Visit Park Page</a>` : '';
//             resultsDiv.innerHTML += parkInfo + visitLink;
//         });
//     }
// }
